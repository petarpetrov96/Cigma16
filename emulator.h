...
public:
    ...
    QString getRegisterFile(); //function to parse the register file for the emulator tab
    QString getMemoryFile();   //function to parse the memory file for the emulator tab
    QString getMemoryAddress(int address);  //function for parsing a single memory address for the emulator tab
    ...
    void loadMemory(QString& data);  //function for loading a raw assembly data into the memory
    static int hex2dec(QChar c);     //function for quick conversion between hexadecimal and decimal
    ...
    QString lastOperation;     //string holding the last operation for the emulator tab
    QString lastOperands;      //string holding the last operands for the emulator tab
    QString lastEffect;        //string holding the last effect for the emulator tab
    QString lastType;          //string holding the last type for the emulator tab
    QString getInstructionList(int type);   //function for parsing the instruction list for the selected architecture
    bool isHalted();           //function for checking if the emulation has been halted
    QString getOutput();       //function for returning the output for the I/O in the emulator tab

private:
    unsigned char memory[65536][4];  //memory values representation
    QString memoryFile;        //string holding the memory file representation in the emulator tab
    ...
    QString output;            //string holding the output of the last instruction (non-empty only after a trap instruction)
